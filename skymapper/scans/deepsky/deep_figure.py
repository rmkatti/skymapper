...
def deepsky_date_survey(save_suffix):
    ...
    for day in days:
        ...
        for i, tupler in enumerate(points_in):
            ...
            if (day in least_plot_days) & (i==0):
                ...
                poll_plot(theta_in, phi_in, hits_in , "least_deep: Day %s"%(day), "least_deep_fig_%s"%(day))
    ...
    phi_in=least_hits_arr[:, 1]
    hits_in=least_hits_arr[:, 2]
    poll_plot(theta_in, phi_in, hits_in , "least_deep: Day %s"%("FINAL"), "least_deep_fig_%s"%("FINAL"))
