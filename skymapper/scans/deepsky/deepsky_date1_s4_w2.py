...
def deepsky_date_survey(save_suffix):
    ...
    savedir = os.path.join(dir, '../../data/deepsky_test/date1_s4_w2/')
    ...
    # Initializations
    skymap = SkyMap(nside=2**9,LVF_theta=FOV_theta, LVF_phi=FOV_phi, cap_theta=pi/4) 
    skyplot1=SkyPlots2()
    ...
    for day in days:
        ...
        if day in plot_days:
            for j,tupler in enumerate(plot_lambda_ranges):
                ...
                savename2="date1_LOWRES_s4_w2_%s_%s" %(day,tupler)
        ...
        if day == 365:
            for j,tupler in enumerate(plot_lambda_ranges):
                ...
                cb=fig1.colorbar(scat)
                cb.set_clim(vmin=0, vmax=250)
                fig1.set_size_inches(20,10)
                fig1.suptitle("Day %s, Nhits=%s, [%s, %s]" %(day, Nhits, tupler[0], tupler[1]) )        

                savename2="date1_s4_w2_cbar_%s_%s" %(day,tupler)
                plt.savefig( savedir + "%s.png" %(savename2) )   
    ...
    fig_sub.savefig( savedir + "%s.png" %("MultiPanel_date1_s1_w2") )   
...
def gen_deepskydate1_points():
    ...
    for day in days:
        ...
        if day % 2==1:
            list_m1= gen_deep3(cent_line_dist, phi,theta_long_1, theta_long_1/2.0, Num_long_far)
            list_m2= gen_deep3(cent_line_dist, phi,theta_long_1/2.0-Strip_width, theta_out_1+Strip_width, 7)
            list_m2 *=3
            list_m3= gen_deep3(cent_line_dist, phi,theta_out_1,0, 3)
        ...
        if day % 2 == 0:
            list_m6= gen_deep3(cent_line_dist, phi, -Strip_width, theta_out_2, 2)
            list_m7= gen_deep3(cent_line_dist, phi, theta_out_2-Strip_width, theta_long_2/2.0+Strip_width, 7)
            list_m7*=3
            list_m8= gen_deep3(cent_line_dist, phi, theta_long_2/2.0, theta_long_2, Num_long_far)
         
            list_m1=[]
            list_m2=[]
            list_m3=[]

        list_o1= gen_deep3(-out_line_dist, phi, theta_out_1, theta_out_2, Num_out  )
        list_o1*=2

        list_o2= gen_deep3(out_line_dist, phi, theta_out_1, theta_out_2, Num_out  )       
        list_o2 *=2

        point_list_i= points_to_nparray(day, list_m1+list_m2+list_m3+list_m6+list_m7+list_m8+list_o2+list_o1)
...
if __name__=='__main__':
    ...
    time1=time.time()
    deepsky_date_survey( 'date1_s4_w2')
