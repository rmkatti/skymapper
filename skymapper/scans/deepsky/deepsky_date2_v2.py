...
def deepsky_date_survey(save_suffix):
    ...
    savedir = os.path.join(dir, '../../data/deepsky_test/date2_v2/')
    ...
    # Initializations
    skymap = SkyMap(nside=2**9,LVF_theta=FOV_theta, LVF_phi=FOV_phi, cap_theta=15*pi/180) 
    skyplot1=SkyPlots2()
    fig_sub, ax_sub= plt.subplots(1, 4, subplot_kw=dict(polar=True))        
    sub_ind=0
    subplot_days=[2, 90, 182, 364]
    #subplot_days=[1,3,7,10]
    plot_days=[2,4, 90, 182, 270, 364]
    ...
    for day in days:
        ...
        if day in plot_days:
            for j,tupler in enumerate(plot_lambda_ranges):
                ...
                savename2="LOWRES_date2_v2_%s_%s" %(day,tupler)
    ...
    fig_sub.savefig( savedir + "%s.png" %("LOWRES_MultiPanel_date2_v2") )   
    
    # Save Dictionary
    skymap.save_lambda_dict(save_suffix)

    array_savename= 'array_date2_v2'
    array1 = lam_dict_to_array(skymap.lambda_dict)
...
if __name__=='__main__':
    ...
    time1=time.time()
    deepsky_date_survey( 'LOWRES_date2_v2')
